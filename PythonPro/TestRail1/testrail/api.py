...
class API(object):
    ...
    print("my code has reached here")
    ...
    def _conf(self):
        ...
        print(config)
        print("reached the point where i am looking for error")
       # print(config['testrail'].get('user_email'))
        _email = os.environ.get(TR_EMAIL) or config['user_email']
        print(config['user_email'])
        print("email ID is " + str(_email))
        _key = os.environ.get(TR_KEY) or config['user_key']
        print(config['user_key'])
        _url = os.environ.get(TR_URL) or config['url']
        print(config['url'])
        ...
        elif config['verify_ssl'] is not None:
            verify_ssl = config['verify_ssl'].lower() == 'true'
    ...
    def _get(self, uri, params=None):
       ...
       # uri = uri % '/index.php?/api/v2/%s'

        print("_get method paramaeters")
        print(uri)
        print(params)
        print(self._auth)
        print(self.headers)
        print(self.verify_ssl)
        uri= "https://priyanewpage.testrail.io//index.php?/api/v2/get_suites/1"
        #uri = self._url+ uri

        print(uri)

        r = requests.get(uri, params=params, auth=self._auth,
