...
def get_project_suite(tr, tr_project, feature_name, feature_description):
    print("hit the project suit method in test rail integration..")
    tr_project_suites = tr.suites()
    print("cursor has reached here..")

    for tr_project_suite in tr_project_suites:
        print(tr_project_suite.name)
        print(tr_project_suite.id)
        print(tr_project_suite.description)
    ...
    raw_project_suite = Suite({
        ...
        'project_id': tr_project
    })
    print('Creating new Suite ', feature_name)
    print("suit name has added successfully...")
    return API.add_suite(raw_project_suite)
